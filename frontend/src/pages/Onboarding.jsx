...
export default function Onboarding() {
  ...
  const submitInfluencerProfile = async () => {
    ...
    try {
      await axios.post(`${API_URL}/influencer/profile`, {
      ...
      await axios.post(`${API_URL}/influencer/profile/social`, {
        youtubeChannelUrl: influencerData.youtubeChannelUrl,
      ...
      if (validWorkLinks.length > 0) {
        await axios.post(`${API_URL}/influencer/profile/work`, {
      ...
      await axios.post(`${API_URL}/influencer/profile/documents`, docFormData, {
        headers: { 'Content-Type': 'multipart/form-data' },
        withCredentials: true,
      });

      await axios.post(`${API_URL}/influencer/submit-verification`, {}, { withCredentials: true });
  ...
  const submitBrandProfile = async () => {
    ...
    try {
      ...
      await axios.post(`${API_URL}/profile/brand`, docFormData, {
        headers: { 'Content-Type': 'multipart/form-data' },
        withCredentials: true,
      });
        ...
        <div className="max-w-4xl mx-auto px-4 py-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-neutral-500 mt-1">Step {currentStep} of {steps.length}</p>
            </div>
                ...
                {currentStep === 1 && (
                  <motion.div key="inf-step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                    <div>
                      ...
                      <p className="text-neutral-600 text-sm">Tell us about yourself</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <textarea name="bio" value={influencerData.bio} onChange={handleInfluencerChange} rows={4}
                        className="w-full border border-neutral-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-neutral-900 resize-none"
                        placeholder="Tell brands about yourself..." />
                      <p className="text-neutral-400 text-sm mt-1">{influencerData.bio.length}/500 characters (min 20)</p>
                    </div>
                  </motion.div>
                )}
                ...
                {currentStep === 2 && (
                  <motion.div key="inf-step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                    <div>
                      ...
                      <p className="text-neutral-600 text-sm">Connect your YouTube presence</p>
                    </div>
                    <div>
                ...
                {currentStep === 3 && (
                  <motion.div key="inf-step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                    <div>
                      ...
                      <p className="text-neutral-600 text-sm">Select categories that match your content</p>
                    </div>
                ...
                {currentStep === 4 && (
                  <motion.div key="inf-step4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                    <div>
                      ...
                      <p className="text-neutral-600 text-sm">Share your best work</p>
                    </div>
                      ...
                      {influencerData.pastWorkLinks.map((link, index) => (
                        <div key={index} className="flex gap-2">
                          <input type="url" value={link} onChange={(e) => handleWorkLinkChange(index, e.target.value)}
                            className="flex-1 border border-neutral-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-neutral-900"
                            placeholder={`Portfolio link ${index + 1}`} />
                          {influencerData.pastWorkLinks.length > 1 && (
                            <button type="button" onClick={() => removeWorkLink(index)}
                              className="p-3 border border-neutral-300 rounded-lg hover:bg-neutral-50">
                              <X className="w-5 h-5" />
                            </button>
                          )}
                        </div>
                ...
                {currentStep === 5 && (
                  <motion.div key="inf-step5" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                    <div>
                      ...
                      <p className="text-neutral-600 text-sm">Upload government-issued ID</p>
                    </div>
                    <div>
                      ...
                      <label htmlFor="document-upload"
                        className={`flex flex-col items-center justify-center w-full h-48 border-2 border-dashed rounded-xl cursor-pointer ${
                          influencerData.document ? 'border-green-500 bg-green-50' : 'border-neutral-300 hover:border-neutral-400 bg-neutral-50'
                        }`}>
                        {influencerData.document ? (
                          <>
                            <CheckCircle className="w-12 h-12 text-green-500 mb-3" />
                            <p className="text-sm font-medium text-green-700">{influencerData.document.name}</p>
                          </>
                        ) : (
                          <>
                  ...
                  <motion.div key="inf-step6" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                    ...
                    <div className="space-y-4">
                      <div className="border rounded-lg p-4">
                        ...
                        <p className="text-sm text-neutral-600"><strong>Name:</strong> {influencerData.displayName}</p>
                      </div>
                      <div className="border rounded-lg p-4">
                        <h3 className="font-semibold mb-2">YouTube</h3>
                        <p className="text-sm text-neutral-600">{influencerData.youtubeChannelUrl}</p>
                      </div>
                      <div className="border rounded-lg p-4">
                        <h3 className="font-semibold mb-2">Categories</h3>
                        <div className="flex flex-wrap gap-2">
                          {influencerData.categoryTags.map(tag => (
                            <span key={tag} className="px-3 py-1 bg-neutral-100 text-sm rounded-full">{tag}</span>
                          ))}
                        </div>
                      </div>
                    </div>
                ...
                {currentStep === 1 && (
                  <motion.div key="brand-step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                    <div>
                      ...
                      <p className="text-neutral-600 text-sm">Tell us about your business</p>
                    </div>
                    <div>
                ...
                {currentStep === 2 && (
                  <motion.div key="brand-step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                    <div>
                      ...
                      <p className="text-neutral-600 text-sm">Additional information</p>
                    </div>
                ...
                {currentStep === 3 && (
                  <motion.div key="brand-step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                    <div>
                      ...
                      <p className="text-neutral-600 text-sm">Upload registration document</p>
                    </div>
                    <div>
                      <input type="file" onChange={handleFileUpload} accept=".pdf,.jpg,.jpeg,.png" className="hidden" id="document-upload" />
                      <label htmlFor="document-upload"
                        className={`flex flex-col items-center justify-center w-full h-48 border-2 border-dashed rounded-xl cursor-pointer ${
                          brandData.document ? 'border-green-500 bg-green-50' : 'border-neutral-300 hover:border-neutral-400 bg-neutral-50'
                        }`}>
                        {brandData.document ? (
                          <>
                            <CheckCircle className="w-12 h-12 text-green-500 mb-3" />
                            <p className="text-sm font-medium text-green-700">{brandData.document.name}</p>
                          </>
                        ) : (
                          <>
                            <Upload className="w-12 h-12 text-neutral-400 mb-3" />
                  ...
                  <motion.div key="brand-step4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                    ...
                    <div className="space-y-4">
                      <div className="border rounded-lg p-4">
                        ...
                        <p className="text-sm text-neutral-600"><strong>Name:</strong> {brandData.companyName}</p>
                        <p className="text-sm text-neutral-600 mt-1"><strong>Website:</strong> {brandData.website}</p>
                        <p className="text-sm text-neutral-600 mt-1"><strong>Industry:</strong> {brandData.industry}</p>
                      </div>
                    </div>
                  </motion.div>
                )}
          ...
          <div className="flex justify-between mt-8 pt-6 border-t">
            {currentStep > 1 ? (
              <button type="button" onClick={prevStep}
                className="flex items-center gap-2 px-6 py-3 border border-neutral-300 rounded-lg hover:bg-neutral-50">
            ...
            {currentStep < steps.length ? (
              <button type="button" onClick={nextStep}
                className="flex items-center gap-2 px-6 py-3 bg-neutral-900 text-white rounded-lg hover:bg-neutral-800">
